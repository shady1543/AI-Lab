...
class ChessBoard(object):
    ...
    @staticmethod
    def format_move_message(team, name, old_row, old_col, new_row, new_col):
        chess_name_mapping = {
            'c': '车',
            'm': '马',
            'p': '炮',
    ...
    def judge_delete_position(self, all_position, clicked_chess):
        ...
        for row, col in all_position:
            ...
            # 3. 判断对方是否可以发起"将军"
            # print(f'clicked_chess.team = {clicked_chess.team}')
    ...
    def move_chess(self, new_row, new_col):
        ...
        # 输出移动信息
        print(self.format_move_message(self.chessboard_map[old_row][old_col].team,
                                       self.chessboard_map[old_row][old_col].name,
                                       old_row, old_col, new_row, new_col))
        print(f'{self.capture_count} 步没有互相吃棋子\n')
    ...
    def judge_attack_general(self, attack_player):
        ...
        """
        # print(f'attact_player = {attack_player}')

        # 1. 找到对方"将"的位置
        general_player = "r" if attack_player == "b" else "b"
        general_position = self.get_general_position(general_player)
        # print(f'general_player = {general_player},general_pos = {general_position}')
